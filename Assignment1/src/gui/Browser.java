...
public class Browser extends Core{
	 ...
	 */
	public Browser(String strSql,JTextField tf) {
		initialize(strSql,tf);
	}

	/**
	 * Initialize the contents of the frame.
	 */
	private void initialize(String strSql,JTextField tf) {
		MySQLConnect();
		...
		tblBrowser.addMouseListener(new MouseAdapter(){
		    public void mouseClicked(MouseEvent evnt) {
		        if (evnt.getClickCount() == 2) {
		        	//tf.setText((String) tblBrowser.getModel().getValueAt(tblBrowser.getSelectedRow(), 0));
		        	tf.setText((String) tblBrowser.getModel().getValueAt(tblBrowser.convertRowIndexToModel(tblBrowser.getSelectedRow()), 0).toString());
		        	tf.requestFocus();
		        	try {
